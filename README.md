...
| File Limits | `nofile` and `nproc` raised in `limits.d` |
...
`vim` · `nano` · `ne` · `micro` · `btop` · `ncdu` · `nmap` · `lynis` · `iftop` · `iotop` · `sysstat` · `mc` · `speedtest-cli` · `jq` · `git` · `git-lfs` · `curl` · `wget`
...
| `%%POSTFIX_DOMAIN%%` | Mail domain |
| `%%POSTFIX_FROM_ADDRESS%%` | Rewritten From address |

`sasl_passwd`, `sender_canonical_maps`, `header_check`, and `generic` are written directly by the script — no templates needed for these.
...
- **MySQL and PostgreSQL tuning** values are auto-calculated from available RAM and CPU at runtime if left blank in the conf file. Suggested values are displayed before prompting so the operator can accept or override them.
- **File limits:** `nofile` and `nproc` values in `/etc/security/limits.conf` are only raised, never lowered. Existing sufficient values are left untouched and logged.
...
- **PyPy:** installed to `/opt` and symlinked into `/usr/local/bin`. The latest PyPy 3.11 release is fetched dynamically at install time via `versions.json`, with a hardcoded fallback version if the fetch fails.
...
- **AppArmor:** enabled and set to enforce mode where profiles exist. The script installs `apparmor-profiles` and `apparmor-profiles-extra`, which provides profiles for MySQL (enforced) and a number of system utilities. Notably, no AppArmor profiles for nginx or sshd are available via apt on Ubuntu 24.04/26.04 — writing custom profiles for these using `aa-genprof` is possible but complex and outside the scope of this script. Both services are hardened by other means (UFW, SSH key-only authentication, Fail2Ban).
